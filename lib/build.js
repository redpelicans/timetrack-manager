...
function build(commit, tag, cb){
  ...
  function dockerCreate(context, cb){
    var config = {
      ...
      Labels:{
        "com.docker.swarm.constraints": "[\"node==" + context.dockerNode + "\"]",
      },
  ...
  function dockerStart(context, container, cb){
    ...
    container.start(function(err){
      ...
      container.inspect(function(err, data){
        ...
        console.log(data);
        console.log("\n\n");
        logte.info("docker", "%s is now running with timetrack rev: %s", context.timetrackContainerName, context.version);
        cb(null, context);
      });
    });
  }
  ...
  function testTimetrack(context, cb){
    setTimeout(function(){
      var options = { hostname: context.host, port: 80, path: '/health', method: 'GET', rejectUnauthorized: false, agent: false, requestCert: true, timeout: 5000 };
      var req = https.request(options, function(res){
        if(res.statusCode != 200) return cb(null, false);
        res.on('data', function(data){ 
          var answer = JSON.parse(data.toString());
          cb(null, answer.health) 
        });
      });
      req.end();
